...
#if ENABLE_MIS_LIGHTING
		segment.color = glm::vec3(0.0f, 0.0f, 0.0f);
		segment.throughtcolor = glm::vec3(1.0f, 1.0f, 1.0f);
#else
		segment.color = glm::vec3(1.0f, 1.0f, 1.0f);
		segment.throughtcolor = glm::vec3(0.0f, 0.0f, 0.0f);
#endif
...
__global__ void rt(int frame, int num_paths, int max_depth,
    ...
    bool trace_shadowray, bool reduce_var, float sintensity, float lightSampleRadius, 
	bool denoise
#if USE_KDTREE
...
#if SHOW_TEXTURE 
				if (material.texid != -1) { col *= texts[material.texid].getColor(uv); }
#endif
				segment.color *= col;
